...
float getDistanceCoveredByTower(float towerPositionX, float towerPositionY, float towerDamageRadius)
{
	...
	float realDamageRadius = towerDamageRadius + float(gameMap->getCellSize()) / 2.0;
...
{
	return getDistanceCoveredByTower(float(float(gameMap->getCellSize()) * (float(towerPositionX) + 0.5)),
									 float(float(gameMap->getCellSize()) * (float(towerPositionY) + 0.5)),
									 towerDamageRadius);
}
